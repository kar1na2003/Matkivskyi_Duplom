...
{
    float fps = pipeline_get_fps_x100() / 100.0f;
    ifx_set_bg_color((color_r[3] << 16) | (color_g[3] << 8) | color_b[3]); // Set background color
    ifx_print_to_buffer(10, 440, "%.1f FPS", fps);                            // Print FPS
...
{
    /* Wait for any previous frame swap to complete (vsync) before
     * presenting the next one.  Without this, the display controller
     * can switch mid-scanout causing the image to appear to jump. */
    while (fb_pending) {
        __NOP();
    }
    fb_pending = true;
...
#endif
            {
                /* Display FPS overlay (bounding boxes removed) */
