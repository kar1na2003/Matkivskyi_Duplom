...
Creates plots in report/plots/.
Uses data from report/data/camera_bench_report.csv.
"""
import csv
import numpy as np
...
CSV_PATH = REPORT_DIR / 'data' / 'camera_bench_report.csv'
...
def main():
    ...
    for fam in families:
        ...
        plt.tight_layout()
        path = OUT_DIR / f'family_{fam}_accuracy.png'
    ...
    for fam in families:
        ...
        path = OUT_DIR / f'family_{fam}_f1_metrics.png'
        fig.savefig(path, dpi=150, bbox_inches='tight')
        plt.close(fig)
        print(f'  Saved: {path}')
    ...
    for fam in families:
        ...
        path = OUT_DIR / f'family_{fam}_degradation.png'
        fig.savefig(path, dpi=150, bbox_inches='tight')
        plt.close(fig)
        print(f'  Saved: {path}')
    ...
    for fam in families:
        ...
        path = OUT_DIR / f'family_{fam}_pareto.png'
        fig.savefig(path, dpi=150, bbox_inches='tight')
        plt.close(fig)
        print(f'  Saved: {path}')
    ...
    path = OUT_DIR / 'best_per_family.png'
    fig.savefig(path, dpi=150, bbox_inches='tight')
    plt.close(fig)
    print(f'  Saved: {path}')
    ...
    path = OUT_DIR / 'stability_ranking.png'
    fig.savefig(path, dpi=150, bbox_inches='tight')
    plt.close(fig)
    print(f'  Saved: {path}')
